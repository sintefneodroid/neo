...
def main():
  ...
  while _environment.is_connected:
    ...
    d4 = messaging.N.Displayable(
        'ScatterPlotDisplayer', np.cos(np.arange(0, 1, step=0.001) * i)
        )
    reaction = messaging.N.Reaction(
        displayables=[d1, d2, d3, d4], serialised_message='this is a serialised_message'
        )
