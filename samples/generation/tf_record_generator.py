...
class TFFeature:
  ...
  def bytes(value):
    ...
    return tf.train.Feature(bytes_list=tf.train.BytesList(value=TFFeature.listify(value)))

  @staticmethod
  def float(value):
    """Returns a float_list from a float / double."""
    return tf.train.Feature(float_list=tf.train.FloatList(value=TFFeature.listify(value)))

  @staticmethod
  def int64(value):
    """Returns an int64_list from a bool / enum / int / uint."""
    return tf.train.Feature(int64_list=tf.train.Int64List(value=TFFeature.listify(value)))
...
def neodroid_tf_example(image_string, label, image_shape, bounding_box):
  ...
  feature = {'height':   TFF.int64([image_shape[0]]),
