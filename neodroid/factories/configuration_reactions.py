      ...
      if configurables:
        if isinstance(input, Reaction):
          ...
          if is_valid_motions:
            ...
            input.motions = construct_configuration_reaction_from_list(input.motions,
                                                                       configurables
                                                                       )
            return input
          ...
          if is_valid_motions:
            ...
            outs.append(construct_configuration_reaction_from_list(input, configurables, env_name=env_name))
        elif isinstance(input, (int, float)):
          outs.append(construct_configuration_reaction_from_list([input], configurables, env_name=env_name))
        elif isinstance(input, (numpy.ndarray, numpy.generic)):
          a = construct_configuration_reaction_from_list(input.astype(float).tolist(),
                                                         configurables,
                                                         env_name=env_name)
          outs.append(a)
      else:
