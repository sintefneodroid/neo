...
from trolls.spaces import VectorSignalSpace, VectorActionSpace, VectorObservationSpace


class VectorUnityEnvironment(DictUnityEnvironment):
    """ """
    ...
    @property
    def description(self) -> VectorEnvironmentDescription:
        """
        ...
        while not self._description:
            self.describe()
        return VectorEnvironmentDescription(
            next(iter(self._description.values())), len(self._description.values())
        )
    ...
    def observation_space(self) -> VectorObservationSpace:
        ...
        return VectorObservationSpace(
            next(iter(self._observation_space.values())),
            len(self._observation_space.values()),
        )
    ...
    def signal_space(self) -> VectorSignalSpace:
        ...
        while not self._signal_space:
            self.describe()
        return VectorSignalSpace(
            next(iter(self._signal_space.values())), len(self._signal_space.values())
        )
