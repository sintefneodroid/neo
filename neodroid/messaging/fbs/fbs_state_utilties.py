...
def deserialise_rigidbody(f_obs) -> Tuple[Any, Any]:
    ...
    return (
        ...
        [
            Range(
                min_value=qt.VelRange().MinValue(),
        ...
        + [
            Range(
                min_value=qt.AngRange().MinValue(),
                max_value=qt.AngRange().MaxValue(),
                decimal_granularity=qt.AngRange().DecimalGranularity(),
            )
            for _ in range(4)
        ],
    )
...
def deserialise_space(flat_space):
    if isinstance(flat_space, list):
        ...
        for space in flat_space:
            if space is not None:
                ran = Range(
                    decimal_granularity=space.DecimalGranularity(),
    ...
    if isinstance(flat_space, str):
        ...
        space = Range(
            decimal_granularity=flat_space.DecimalGranularity(),
