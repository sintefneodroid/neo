...
def serialise_unobservables(B: Builder, input_reaction) -> Union[int, bytearray]:
    ...
    if unobservables:
        ...
        if not unobservables:
            raise NoUnobservablesException(
                "You probably did not receive any unobservables, check you simulator "
...
def serialise_displayables(B: Builder, input_reaction) -> Union[int, bytearray]:
    ...
    if input_reaction.displayables is not None:
        for input_displayable in input_reaction.displayables:
            ...
            if isinstance(input_value, float) or isinstance(input_value, int):
                ...
                isinstance(input_value, (list, numpy.ndarray))
                and isinstance(input_value[0], Sequence)
                and len(input_value[0]) == 3
            ):
    ...
    displayables_length = len(displayables_offsets)
    F.FReactionStartConfigurationsVector(B, displayables_length)
    for offset in displayables_offsets:
        B.PrependUOffsetTRelative(offset)
    return B.EndVector(displayables_length)
...
def serialise_configurations(B: Builder, input_reaction) -> Union[int, bytearray]:
    ...
    configurations_length = len(configurations_offsets)
    F.FReactionStartConfigurationsVector(B, configurations_length)
    for input_configuration in configurations_offsets:
        B.PrependUOffsetTRelative(input_configuration)
    return B.EndVector(configurations_length)
