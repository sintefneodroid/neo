...
class EnvironmentState(object):
  ...
  def __init__(self, time_since_reset: float = 0,
               total_energy_spent_since_reset=0, actors=[], observers=[],
  ...
  def __repr__(self):
    observers_str = ''.join(
        [str(observer.__repr__()) for observer in self._observers])
    actors_str = ''.join([str(actor.__repr__()) for actor in self._actors])

    return '<EnvironmentState>\n' + \
           '  <time_since_reset>' + str(
        self._time_since_reset) + '</time_since_reset>\n' + \
           '  <total_energy_spent_since_reset>' + str(
        self._total_energy_spent_since_reset) + '</total_energy_spent_since_reset>\n' + \
           '  <reward_for_last_step>' + str(
        self._reward_for_last_step) + '</reward_for_last_step>\n' + \
           '  <Actors>\n' + \
           actors_str + \
           '  </Actors>\n' + \
