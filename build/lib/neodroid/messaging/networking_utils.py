...
def start_connect_thread(tcp_ip_address='127.0.0.1', tcp_port=5555,
...
def start_send_msg_thread(stream, action, on_connected_callback):
  thread = Thread(target=send_reaction,
                  args=(stream, action, on_connected_callback))
  thread.daemon = True  # Terminate with the rest of the program, is a Background Thread
  thread.start()


def start_receive_environment_state_thread(stream, on_connected_callback):
  thread = Thread(target=receive_environment_state,
                  args=(stream, on_connected_callback))
  thread.daemon = True  # Terminate with the rest of the program, is a Background Thread
  thread.start()
